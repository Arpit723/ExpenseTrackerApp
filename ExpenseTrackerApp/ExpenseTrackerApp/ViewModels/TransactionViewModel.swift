...
class TransactionViewModel: ObservableObject {
  ...
  private func setupBindings() {
    ...
    NotificationCenter.default.publisher(for: .transactionDataRefreshed)
      .receive(on: DispatchQueue.main)
      .sink { [weak self] _ in self?.loadTransactions() }
      .store(in: &cancellables)
  }
  ...
  func addTransactionAsync(_ transaction: Transaction) async {
    do {
      try await dataService.addTransaction(transaction)
    } catch {
      self.error = AppError.from(firebaseError: error)
  ...
  func updateTransactionAsync(_ transaction: Transaction) async {
    do {
      try await dataService.updateTransaction(transaction)
    } catch {
      self.error = AppError.from(firebaseError: error)
